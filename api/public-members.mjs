...
export default async function handler(_req, res) {
  ...
  if (publicMembersResult.error) {
    return res.status(500).json({ ok: false, error: 'members_query_failed', message: publicMembersResult.error.message });
  }
  if (tiersResult.error) {
    return res.status(500).json({ ok: false, error: 'tiers_query_failed', message: tiersResult.error.message });
  }
  ...
  // Member list can change whenever vetting happens. Shorter cache than the
  // tier list, but still cached at the edge to protect Supabase under
  // launch-day traffic.
