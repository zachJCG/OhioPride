...
export default async function handler(req, res) {
  ...
  if (appType === 'internship') return await handleInternship(supabase, req, res, body);
  return await handleVolunteer(supabase, req, res, body);
}

// -----------------------------------------------------------------------------
// Volunteer path
// -----------------------------------------------------------------------------
async function handleVolunteer(supabase, req, res, body) {
  ...
  if (!email || !EMAIL_RE.test(email)) return res.status(400).json({ ok: false, error: 'valid_email_required' });

  const zip = clean(body.zip, 10);
  if (zip && !/^\d{5}(-\d{4})?$/.test(zip)) return res.status(400).json({ ok: false, error: 'invalid_zip' });
  ...
  return res.status(200).json({ ok: true, id: data?.id || null, kind: 'volunteer' });
}

// -----------------------------------------------------------------------------
// Internship path
// -----------------------------------------------------------------------------
async function handleInternship(supabase, req, res, body) {
  const first_name = clean(body.first_name, 100);
  const last_name  = clean(body.last_name,  100);
  const email      = clean(body.email,      320)?.toLowerCase() || null;
  ...
  if (!first_name || !last_name) return res.status(400).json({ ok: false, error: 'name_required' });
  if (!email || !EMAIL_RE.test(email)) return res.status(400).json({ ok: false, error: 'valid_email_required' });
  if (!position) return res.status(400).json({ ok: false, error: 'position_required' });
  ...
  if (resume && !URL_RE.test(resume))       return res.status(400).json({ ok: false, error: 'invalid_url' });
  if (portfolio && !URL_RE.test(portfolio)) return res.status(400).json({ ok: false, error: 'invalid_url' });
  ...
  return res.status(200).json({ ok: true, id: data?.id || null, kind: 'internship' });
}
