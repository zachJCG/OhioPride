...
const json = (res, status, body, cacheSeconds = 300) => {
  if (cacheSeconds > 0) {
    res.setHeader('Cache-Control', `public, max-age=${cacheSeconds}, s-maxage=${cacheSeconds * 2}, stale-while-revalidate=1800`);
  }
...
export default async function handler(req, res) {
  ...
  if (slug) {
    ...
    if (billRes.error)      return json(res, 500, { ok: false, error: billRes.error.message }, 0);
    if (!billRes.data)      return json(res, 404, { ok: false, error: 'bill_not_found' }, 60);
    if (pipelineRes.error)  return json(res, 500, { ok: false, error: pipelineRes.error.message }, 0);
    if (rollCallsRes.error) return json(res, 500, { ok: false, error: rollCallsRes.error.message }, 0);
  ...
  if (billsRes.error)    return json(res, 500, { ok: false, error: billsRes.error.message }, 0);
  if (pipelineRes.error) return json(res, 500, { ok: false, error: pipelineRes.error.message }, 0);
