...
export default async function handler(req, res) {
  ...
  if (!sb) return json(res, 500, { ok: false, error: 'server_misconfigured' });
  ...
  if (adminErr || !isAdminRow) return json(res, 403, { ok: false, error: 'not_admin' });
  ...
  const r = {};
  results.forEach((result, i) => { r[keys[i]] = result; });
