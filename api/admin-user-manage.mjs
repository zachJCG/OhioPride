...
// Public origin for auth redirect links. SITE_URL is the explicit override;
// VERCEL_PROJECT_PRODUCTION_URL is the bare production hostname Vercel sets
// on every deployment.
function siteOrigin() {
...
export default async function handler(req, res) {
  ...
  if (!m) return json(res, 401, { ok: false, error: 'missing_bearer' });
  const jwt = m[1];
  ...
  if (!userSb) return json(res, 500, { ok: false, error: 'server_misconfigured' });
  ...
  if (permErr) {
    ...
    return json(res, 500, { ok: false, error: 'permission_check_failed' });
  }
  if (!allowed) return json(res, 403, { ok: false, error: 'not_authorized' });

  const adminSb = adminClient();
  if (!adminSb) {
    return json(res, 503, { ok: false, error: 'service_role_unconfigured' });
  }

  try {
    if (action === 'invite') {
      const fullName = body.full_name ? String(body.full_name).trim() : null;
      const redirectTo = siteOrigin() + '/admin/login';
      ...
      if (error) {
        ...
        if (msg.includes('already') || msg.includes('exists')) {
          return json(res, 200, { ok: true, invite_sent: false, note: 'user_already_exists' });
        }
        throw error;
      }
      return json(res, 200, { ok: true, invite_sent: true, user_id: data && data.user ? data.user.id : null });
    }

    if (action === 'update_email') {
      const newEmail = String((body && body.new_email) || '').trim().toLowerCase();
      if (!validEmail(newEmail)) return json(res, 400, { ok: false, error: 'invalid_new_email' });
      if (newEmail === email)     return json(res, 200, { ok: true, no_change: true });
      ...
      return json(res, 200, { ok: true, auth_updated: !!authUser, row_updated: true });
    }
    ...
    if (action === 'set_password') {
      ...
      if (!authUser) {
        ...
        if (error) throw error;
        return json(res, 200, { ok: true, created: true, user_id: data && data.user ? data.user.id : null });
      }
      const { error } = await adminSb.auth.admin.updateUserById(authUser.id, { password: newPassword });
      if (error) throw error;
      return json(res, 200, { ok: true, updated: true, user_id: authUser.id });
    }
    ...
    if (action === 'send_password_reset') {
      ...
      if (error) throw error;
      return json(res, 200, { ok: true, reset_sent: true });
    }

    return json(res, 400, { ok: false, error: 'unknown_action', action });
  } catch (err) {
    console.error(`admin-user-manage[${action}] failed:`, err);
    return json(res, 500, { ok: false, error: 'operation_failed', detail: err.message });
  }
}
