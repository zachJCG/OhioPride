...
async function requireCommsAdmin(req, res) {
  const auth = req.headers['authorization'] || '';
  const m = auth.match(/^Bearer (.+)$/i);
  if (!m) return { error: json(res, 401, { ok: false, error: 'missing_bearer' }) };

  const sb = userClient(m[1]);
  if (!sb) return { error: json(res, 500, { ok: false, error: 'server_misconfigured' }) };
  ...
  if (error) {
    ...
    return { error: json(res, 500, { ok: false, error: 'permission_check_failed' }) };
  }
  if (!allowed) return { error: json(res, 403, { ok: false, error: 'not_authorized' }) };
  return { sb };
}

export default async function handler(req, res) {
  const gate = await requireCommsAdmin(req, res);
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      return json(res, 502, { ok: false, error: 'mailerlite_error', detail: err.message });
    }
  }
  ...
  if (req.method === 'POST') {
    if (!isConfigured()) {
      return json(res, 503, { ok: false, error: 'mailerlite_unconfigured' });
    }
    ...
    if (!subject) return json(res, 400, { ok: false, error: 'subject_required' });
    if (!html)    return json(res, 400, { ok: false, error: 'content_required' });
    if (!groupIds.length) return json(res, 400, { ok: false, error: 'group_required' });
    if (!from || !EMAIL_RE.test(from)) {
      return json(res, 400, { ok: false, error: 'valid_from_required',
        detail: 'Set MAILERLITE_FROM_EMAIL to a verified sender, or pass a valid "from".' });
    }
    if (replyTo && !EMAIL_RE.test(replyTo)) {
      return json(res, 400, { ok: false, error: 'invalid_reply_to' });
    }
    ...
    try {
      ...
      return json(res, 200, { ok: true, campaign_id: campaign?.id || null });
    } catch (err) {
