...
export default async function handler(_req, res) {
  ...
  if (legRes.error)   return json(res, 500, { ok: false, error: legRes.error.message  }, 0);
  if (sponsRes.error) return json(res, 500, { ok: false, error: sponsRes.error.message }, 0);
  if (rollRes.error)  return json(res, 500, { ok: false, error: rollRes.error.message }, 0);
  if (excRes.error)   return json(res, 500, { ok: false, error: excRes.error.message  }, 0);
