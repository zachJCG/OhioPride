...
import { countyLabel } from './endorsement-race.mjs';
...
function decorate(row) {
  ...
  return {
    ...
    /* A judgeship or a county row office has no district; the county is the
     * jurisdiction, so it takes the district's place on the card and the
     * profile ("Probate Court, Montgomery County"). An editorial district
     * still wins, and a seat that has both shows the district. */
    district: content?.district || row.district || countyLabel(row.county),
    county: row.county || null,
    electionYear: row.election_year || null,
    website: normalizeUrl(row.website),
    bio: row.bio || null,
    endorsedAt: row.endorsed_at || null,
    /* Only an explicit "yes" from the questionnaire. "prefer_not_to_say" and
     * blank never render the ring, so the site can never out anyone. */
    isOut: row.is_out === 'yes',
    level: levelOf(row),
...
export async function getEndorsements() {
  const query = new URLSearchParams({
    select: 'id,candidate_name,pronouns,office_sought,district,county,election_year,website,bio,is_out,endorsed_at,endorsement_path',
