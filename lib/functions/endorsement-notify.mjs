...
import { raceLabel, submittedByLabel } from '../endorsement-race.mjs';
...
export default async (req) => {
  ...
  const { ok } = await notifySubmission({
    ...
    fields: {
      ...
      // Office, district, county, and the year (or "special election"), from
      // the same helper the admin queue and the board packet use.
      Race: raceLabel(row, ' — ', { includeCycle: true }),
      Party: row.party,
      ...
      'Submitted by': submittedByLabel(row),
      'Submitter email': row.submitted_by_kind && row.submitted_by_kind !== 'candidate' ? row.submitted_by_email : null,
      Photo: row.photo_path ? 'Attached (open the application to view it)' : 'None',
      Status: row.status,
      Bio: row.bio,
    },
    replyTo: row.email,
