...
const COLUMNS = {
  ...
  amount:     ['amount', 'amount ', 'contribution amount', 'total amount', 'total'],
...
export default async (req) => {
  ...
  for (const row of rows.slice(1)) {
    ...
    if (isContributions) {
      ...
      if (!receipt || !(dollars > 0)) continue;
      const recurringRaw = get(row, 'recurring').toLowerCase();
      const monthly = recurringRaw && !['', '0', 'no', 'false', 'one-time', 'once', 'n/a'].includes(recurringRaw);
      contributions.push({
        email,
        receipt_id: receipt,
        lineitem_id: get(row, 'lineitem_id') || null,
        amount_cents: Math.round(dollars * 100),
        contributed_at: get(row, 'date') ? new Date(get(row, 'date')).toISOString() : null,
