...
export default async (req) => {
  ...
  for (const rec of records) {
    ...
    for (const f of FILL_FIELDS) {
      const v = f === 'state' ? norm(rec[f]).toUpperCase().slice(0, 2) : f === 'zip' ? norm(rec[f]).replace(/[^0-9]/g, '').slice(0, 5) : norm(rec[f]);
      if (v && !cur[f]) cur[f] = v;
    }
  ...
  if (isContributions && contributions.length) {
    ...
    for (const p of sync.problems) {
      conflicts.push({ email: '', field: p.kind, existing: p.lineitem || '', incoming: p.message || '' });
    }
  }
