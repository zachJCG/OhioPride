...
export default async (req, _ctx) => {
  ...
  const promises = {
    ...
    // Last real ActBlue sync (RLS: donors:read), so the dashboard can say
    // how fresh the member numbers are.
    syncLast:    sb.from('actblue_sync_runs')
                   .select('started_at, finished_at, status, trigger, founding_inserted, donors_inserted, error')
                   .eq('dry_run', false).order('started_at', { ascending: false }).limit(1),
  };
  ...
  const syncRow = (okValue('syncLast', []).data || [])[0] || null;
  const actblue_last_sync = syncRow ? {
    at: syncRow.finished_at || syncRow.started_at,
    when: relative(syncRow.finished_at || syncRow.started_at),
    status: syncRow.status,
    trigger: syncRow.trigger,
    founding_inserted: syncRow.founding_inserted || 0,
    donors_inserted: syncRow.donors_inserted || 0,
  ...
  (okValue('endRecent', []).data || []).forEach(e => {
    recent.push({
      ...
      ts: e.created_at,
      href: '/admin/endorsements/' + encodeURIComponent(e.id)
    });
  });
  (okValue('fmRecent', []).data || []).forEach(f => {
    recent.push({
      ...
      ts: f.created_at,
      href: '/admin/contacts?seg=founding'
    });
  });
