...
export default async (req) => {
  ...
  /* ---- auth ------------------------------------------------------------- */
  // Authentication comes BEFORE the environment report: an unauthenticated
  // caller should learn nothing about which server secrets are configured.
  // This endpoint writes to the donor database, so it never runs unauthenticated.
  // With CRON_SECRET unset the cron cannot authenticate either; that is the
  // point, and the 401 says so rather than failing open.
  ...
  if (bearer && cronSecret && constantTimeEqual(bearer, cronSecret)) {
    ...
    return json(401, {
      ...
      hint: cronSecret
        ? 'Send the Vercel cron secret, or an admin access token with donors:write.'
        : 'CRON_SECRET is not set in this project, so the hourly cron cannot authenticate and this endpoint refuses every unauthenticated call. Set it in Vercel and redeploy.',
    });
  }
  if (!cronSecret) {
    warnings.push('CRON_SECRET is not set, so the hourly cron cannot run this sync. Set it in Vercel and redeploy; only signed-in admins can sync until then.');
  }
  ...
  if (until && !since) {
    // Silently running the incremental window would look like a backfill that
    // did nothing rather than a mistyped request.
    return json(400, { ok: false, error: 'bad_param', message: 'until needs since; pass both to run a backfill' });
  }
  let types = ALL_TYPES;
  if (params.types) {
  ...
  let sub = { founding_adopted: 0, donors_adopted: 0, donors_updated: 0, donors_deduped: 0, founding_refunded: 0, donors_refunded: 0, founding_cancelled: 0, donors_cancelled: 0, refunds_unmatched: 0, cancellations_unmatched: 0, errors: 0 };
  ...
  try {
    for (const [i, ch] of chunks.entries()) {
      ...
      if (types.includes('paid_contributions')) {
        ...
        sub = sum(sub, { founding_adopted: r.counts.founding_adopted, donors_adopted: r.counts.donors_adopted, donors_updated: r.counts.donors_updated, donors_deduped: r.counts.donors_deduped, errors: r.counts.errors });
      ...
      if (types.includes('cancelled_recurring_contributions')) {
        ...
        const r = await applyCancellations(admin, rows, { dryRun, foundingMatch });
