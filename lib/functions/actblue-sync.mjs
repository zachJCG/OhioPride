...
export default async (_req, _context) => {
  ...
  for (const r of rows) {
    ...
    // Lineitem ID first: it is unique per installment, while receipt IDs
    // repeat across a recurring series. This also matches the convention the
    // 2026-05 backfill used for actblue_contribution_id.
    const contribId = pick(r, 'Lineitem ID', 'Contribution ID') || receiptId;
  ...
  if (donorRows.length) {
    // Dedupe key = email + (receipt ID, falling back to lineitem ID for rows
    // ActBlue exports without a receipt). Pre-read existing rows by BOTH id
    // columns so lineitem-only rows from earlier runs are found too;
    // otherwise every hourly run inside the window would re-insert them.
    const ids = [...new Set(donorRows.flatMap(d => [d.actblue_receipt_id, d.actblue_contribution_id]).filter(Boolean))];
    const seen = new Set();
    const pairKey = (email, receipt, contrib) => `${String(email || '').toLowerCase()}|${receipt || contrib}`;
    for (let i = 0; i < ids.length; i += 100) {
      const chunk = ids.slice(i, i + 100);
      for (const col of ['actblue_receipt_id', 'actblue_contribution_id']) {
        ...
        for (const d of data || []) {
          seen.add(pairKey(d.email, d.actblue_receipt_id, d.actblue_contribution_id));
        }
      }
    }
    const fresh = [];
    for (const d of donorRows) {
      const key = pairKey(d.email, d.actblue_receipt_id, d.actblue_contribution_id);
