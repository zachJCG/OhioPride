...
export default async (req, _context) => {
  ...
  // unsubscribed person back to active. Fields the caller left blank are
  // dropped from the row entirely: an explicit null in an upsert overwrites
  // on email conflict, so a name-less re-subscribe (the homepage band only
  // asks for email + ZIP) would erase a stored name or phone.
  const row = {
    email,
  ...
  // Tell staff. Also best-effort — the subscriber is already saved.
  const subscriberName = [row.first_name, row.last_name].filter(Boolean).join(' ');
  await notifySubmission({
    kind: 'newsletter',
    title: `New newsletter signup${subscriberName ? ': ' + subscriberName : ''}`,
    fields: {
      Name: subscriberName,
      Email: email,
      Phone: row.phone,
    ...
    adminLabel: 'Open contacts in the admin console',
  });
