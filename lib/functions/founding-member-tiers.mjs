...
export default async (_req, _context) => {
  ...
  const COLUMNS = 'name, slug, amount_cents, recurrence, match_mode, description, display_order, actblue_refcode_prefix';
  let { data, error } = await supabase
    .from('founding_member_tiers')
    .select(`${COLUMNS}, actblue_url`)
    .eq('is_active', true)
    .order('display_order', { ascending: true });

  // actblue_url reached production only on 2026-09-08. A database without the
  // column (an older fork, a preview branch) still gets the tier cards; they
  // just render without the ActBlue link instead of the whole page's tier
  // section failing over to static markup.
  if (error && /actblue_url/.test(error.message || '')) {
