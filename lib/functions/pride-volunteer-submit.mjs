...
export default async (req, _context) => {
  ...
  if (!preferred_region) {
    return jsonResponse(400, { ok: false, error: 'region_required' });
  }
  ...
  if (error) {
    ...
    if (error.code === '23505') {
      return jsonResponse(409, { ok: false, error: 'already_signed_up' });
    }
  ...
  // Staff notification. Best-effort — the row is already saved.
  await notifySubmission({
    kind: 'volunteer',
    title: `New Pride road-tour volunteer: ${first_name} ${last_name}`,
    fields: {
      Name: `${first_name} ${last_name}`,
      Email: email,
      Phone: row.phone,
      Location: [row.city, row.zip].filter(Boolean).join(', '),
      Region: row.preferred_region,
      Events: row.events_interested,
      Roles: row.roles_interested,
      'Can travel': row.can_travel,
      Vehicle: has_vehicle ? `Yes${vehicle_capacity ? ` (seats ${vehicle_capacity})` : ''}` : 'No',
      'T-shirt': row.tshirt_size,
      Accessibility: row.accessibility_needs,
      'Emergency contact': [row.emergency_contact_name, row.emergency_contact_phone].filter(Boolean).join(' — '),
      'Heard via': row.how_heard,
      Notes: row.notes,
    },
    replyTo: email,
    adminPath: '/admin/pride',
    adminLabel: 'Open Pride events in the admin console',
  });
