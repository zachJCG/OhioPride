...
import { syncSubscriberSafe } from '../mailerlite.mjs';
...
async function handleVolunteer(supabase, req, body) {
  ...
  // Staff notification. Best-effort — the volunteer row is already saved.
  await notifySubmission({
    kind: 'volunteer',
    title: `New volunteer signup: ${first_name} ${last_name}`,
    fields: {
      Name: `${first_name} ${last_name}`,
      Pronouns: row.pronouns,
      Email: email,
      Phone: row.phone,
      Location: [row.city, row.county, row.zip].filter(Boolean).join(', '),
      'Registered voter': row.registered_voter,
      Interests: row.interests,
      Skills: row.skills,
      Availability: row.availability,
      'Time commitment': row.time_commitment,
      'Prior experience': row.prior_campaign_notes,
      'Founding member': row.is_founding_member,
      'Heard via': row.referral_source,
      Notes: row.additional_notes,
    },
    replyTo: email,
    adminPath: '/admin/volunteers',
    adminLabel: 'Open volunteers in the admin console',
  });
...
async function handleInternship(supabase, req, body) {
  ...
  // Staff notification. Best-effort — the application row is already saved.
  await notifySubmission({
    kind: 'volunteer',
    title: `New internship application: ${first_name} ${last_name}`,
    fields: {
      Name: `${first_name} ${last_name}`,
      Pronouns: row.pronouns,
      Email: email,
      ...
      Position: position,
      Term: term,
      'Start date': row.start_date_pref,
      'Weekly hours': row.weekly_hours,
      'Credit hours': row.credit_hours,
      Institution: row.institution,
      Program: row.program_major,
      'Class year': row.class_year,
      'Faculty sponsor': [row.faculty_sponsor_name, row.faculty_sponsor_email].filter(Boolean).join(' — '),
      Resume: row.resume_url,
      Portfolio: row.portfolio_url,
      Statement: statement,
      'Prior experience': row.prior_experience,
      'Why Ohio Pride': row.why_ohio_pride,
      'Heard via': row.referral_source,
    },
    replyTo: email,
    adminPath: '/admin/internships',
    adminLabel: 'Open internships in the admin console',
  });
