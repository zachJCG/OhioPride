...
export function parseActBlueDate(value, timeZone = 'America/New_York') {
  ...
  const [Y, MO, D, H, MI, S] = [+y, +mo, +d, +h, +mi, +se];
  // The regex accepts any two digits per field and Date.UTC would roll them
  // over, turning "2026-02-30" into March 2 rather than rejecting it. A date
  // we cannot trust must read as missing, not as a different day.
  if (MO < 1 || MO > 12 || D < 1 || D > 31 || H > 23 || MI > 59 || S > 59) return null;
  const probe = new Date(Date.UTC(Y, MO - 1, D));
  if (probe.getUTCMonth() !== MO - 1 || probe.getUTCDate() !== D) return null;
  return wallClockToIso({ y: Y, mo: MO, d: D, h: H, mi: MI, s: S }, timeZone);
}
...
export async function fetchActBlueCsv(opts) {
  ...
  let wait = pollMs;
  // Retries count FAILURES, not polls. Counting polls meant a big export that
  // legitimately takes a minute to build had spent its whole allowance on
  // healthy "in_progress" answers, so the first transient 429 aborted the run.
  let retries = 0;
  for (;;) {
    ...
    if (!res.ok) {
      ...
      if ((res.status >= 500 || res.status === 429) && retries++ < 8) {
...
/**
 * How a new member is shown on the public roster: "First L.", the convention
 * the hand-entered rows follow. Never blank when a first name exists, because
 * founding_members_public falls back to the full legal name when display_name
 * is empty, and an organizer vetting a row should not publish more than the
 * roster convention.
 */
export function publicDisplayName({ first_name: first, last_name: last, full_name: full } = {}) {
  const f = norm(first);
  const l = norm(last);
...
export async function reconcileContributions(admin, contributions, opts = {}) {
  ...
  const counts = {
    ...
    donors_inserted: 0, donors_updated: 0, donors_adopted: 0, donors_skipped: 0, donors_deduped: 0,
  ...
  /* Donors rows already holding one of this batch's ids and NOT owned by a
   * founding member. A new seat keyed on such an id would make the fan-out
   * trigger's insert collide with the unique key on donors.actblue_contribution_id
   * and fail the whole row, every run, forever. */
  const strayDonorByKey = new Map();
  for (const d of await readIn(admin, 'donors', 'id, actblue_contribution_id, founding_member_id', 'actblue_contribution_id', allIds)) {
    if (!d.founding_member_id && d.actblue_contribution_id) strayDonorByKey.set(d.actblue_contribution_id, d);
  }

  /* Which emails already had a contact BEFORE this run wrote anything. The
   * link_or_create_contact triggers fire on every insert below, so reading
   * this after the fact would report every genuinely new contact as merely
   * "enriched" and make a live run disagree with its own dry run. */
  const contactsBefore = new Set(
    emails.length
      ? (await readIn(admin, 'contacts', 'email', 'email', emails)).map((c) => normEmail(c.email))
      : []
  );
  ...
  for (const c of rows) {
    ...
    let byId = (c.lineitem_id && fmByKey.get(c.lineitem_id)) || (c.receipt_id && fmByKey.get(c.receipt_id)) || null;
    const byEmail = !byId && c.email ? fmByEmail.get(c.email) : null;

    // A seat entered by hand or by an early import carries no ActBlue key at
    // all (17 live rows). Matching it only by email would read this CSV row as
    // a SECOND gift and record the membership payment twice: once through the
    // fan-out trigger, once as an "additional gift". Same amount within the
    // adopt window means it is that payment, so adopt the keys instead.
    if (!byId && byEmail && !byEmail.pending && byEmail.id
        && !byEmail.actblue_contribution_id && !byEmail.actblue_receipt_id
        && byEmail.amount_cents === c.amount_cents
        && byEmail.contributed_at && c.contributed_at
        && Math.abs(new Date(byEmail.contributed_at).getTime() - new Date(c.contributed_at).getTime()) <= ADOPT_WINDOW_MS) {
      byId = byEmail;
    }
    ...
    fmInserts.push(c);
    // Claim the email for this batch straight away. The inserts happen after
    // this loop, so without a placeholder a person whose two founding-refcode
    // gifts land in the same run (every 180-day first run, every backfill)
    // would be queued twice and end up with two seats: founding_members has no
    // unique on email to catch it. Rows are sorted chronologically, so the
    // earliest gift takes the seat and the rest become additional gifts.
    if (c.email) fmByEmail.set(c.email, { id: null, email: c.email, contributed_at: c.contributed_at, pending: true });
  }
  ...
  for (const c of fmInserts) {
    // A stray donors row already holding this key would make the fan-out
    // trigger's insert violate donors.actblue_contribution_id and fail the
    // seat on every run. Key the seat on its receipt instead and report it.
    let key = c.lineitem_id || c.receipt_id;
    const stray = strayDonorByKey.get(key);
    if (stray) {
      if (c.receipt_id && key !== c.receipt_id && !strayDonorByKey.has(c.receipt_id)) key = c.receipt_id;
      problems.push({ kind: 'lineitem_clash', lineitem: c.lineitem_id, receipt: c.receipt_id, donor_id: stray.id });
    }
    const row = {
      full_name: c.full_name || c.email || 'Anonymous',
      email: c.email,
      // founding_members_public falls back to full_name when display_name is
      // empty, so a row left blank here publishes a legal name the moment
      // someone vets it. The roster convention is "First L.".
      display_name: publicDisplayName(c),
  ...
  if (toDonorPass.length) {
    ...
    for (const { c, reason } of toDonorPass) {
      ...
      // A different lineitem is a different payment, so a row that carries its
      // own lineitem may only adopt a receipt-keyed row. A receipt-only row
      // (an older export with no Lineitem ID column) has no such evidence and
      // may adopt the lineitem-keyed row for the same receipt, amount and time
      // rather than inserting the payment a second time.
      const same = candidates.find((r) => !claimed.has(r.id)
        && (!c.lineitem_id || !isNumericId(r.actblue_contribution_id))
    ...
    if (dryRun) {
      ...
      for (const part of chunk(inserts, 100)) {
        ...
        const wrote = data?.length ?? part.length;
        counts.donors_inserted += wrote;
        // ignoreDuplicates swallows a row whose key another writer already
        // holds. Counting the gap keeps "inserted" honest instead of reporting
        // payments that were never written.
        if (wrote < part.length) counts.donors_deduped += part.length - wrote;
      }
    }
  }
  ...
  if (emails.length) {
    ...
    for (const [email, rec] of byEmail) {
      ...
      // A contact this run brought into existence counts as created even when
      // the link_or_create_contact trigger got there first and this pass only
      // fills the rest in. Reading it any other way makes a live run disagree
      // with its own preview.
      const isNew = !contactsBefore.has(email);
      if (!cur) {
      ...
      if (!Object.keys(patch).length) {
        if (isNew) counts.contacts_created++;
        continue;
      }
      if (!dryRun) {
 ...
 */
export async function applyCancellations(admin, cancellations, { dryRun = false, foundingMatch = DEFAULT_FOUNDING_MATCH } = {}) {
  ...
  const FM_SELECT = 'id, email, actblue_contribution_id, actblue_receipt_id, recurrence, recurring_cancelled_at, refcode, amount_cents';
  ...
  for (const c of cancellations) {
    ...
    if (!fmTarget && c.email) {
      // Falling back to "this person's monthly seat" is only safe when the
      // cancelled series could BE that seat. A donor who gives monthly on a
      // founding refcode and separately cancels a $25 monthly gift would
      // otherwise have their membership marked cancelled while ActBlue keeps
      // charging it. Require the cancelled series to look founding, and when
      // its refcode is blank require the pledge amount to line up.
      const cancelledRefcode = norm(c.refcode);
      const refcodeLooksFounding = cancelledRefcode
        ? isFoundingRefcode(cancelledRefcode, foundingMatch)
        : null;
      if (refcodeLooksFounding !== false) {
        fmTarget = fmRows.find((f) => normEmail(f.email) === c.email
          && f.recurrence === 'monthly'
          && (refcodeLooksFounding === true
            || c.recurrence_amount_cents == null
            || f.amount_cents === c.recurrence_amount_cents)) || null;
      }
    }
    if (fmTarget) {
      hit = true;
