...
// Votes are endorse / decline / abstain, matching the endorsement_reviews
// CHECK and the admin vote bar. Keep the three in step: the packet is the
// record the board signs off on.
const label = (k) => ({ endorse: 'Endorse', decline: 'Decline', abstain: 'Abstain' }[k] || k);
...
function Packet({ app, questions, reviews, activity }) {
  ...
  for (const r of reviews) {
    if (r.vote === 'endorse') buckets.endorse++;
    else if (r.vote === 'decline') buckets.decline++;
  ...
  return (
    <Page size="LETTER" style={s.page} wrap>
      ...
      <View style={s.tallyRow}>
        <View style={s.tallyBox}><Text style={{ fontSize: 14, fontFamily: 'Helvetica-Bold' }}>{buckets.endorse}</Text><Text style={s.lbl}>Endorse</Text></View>
        <View style={s.tallyBox}><Text style={{ fontSize: 14, fontFamily: 'Helvetica-Bold' }}>{buckets.decline}</Text><Text style={s.lbl}>Decline</Text></View>
