...
const json = (status, body, cacheSeconds = 120) =>
  new Response(JSON.stringify(body), {
    status,
    headers: {
      'content-type': 'application/json',
      'cache-control': `public, max-age=${cacheSeconds}, s-maxage=${cacheSeconds * 2}, stale-while-revalidate=900`,
    },
  });
...
export default async (_req, _context) => {
  ...
  if (eventsRes.error) return json(500, { ok: false, error: eventsRes.error.message }, 0);
  if (statusRes.error) return json(500, { ok: false, error: statusRes.error.message }, 0);
...
export const config = { path: '/.netlify/functions/pride-events' };
