...
/* Composite score (0-100).
   Evidence-only. Every lawmaker starts from the same flat 50 baseline
   regardless of party or caucus. The grade is driven entirely by what
   each lawmaker actually did, sponsored, or said on the record.

   Evidence weights (April 2026 rebalance):
     V (votes)         x6   (range -30 to +30)  — binding action
     S (sponsorship)   x3   (range -15 to +15)  — public commitment
     N (news/quotes)   x1   (range  -5 to  +5)  — on-record statements
...
function calcScore(member) {
  ...
  var weighted = (sub.v * 6) + (sub.s * 3) + (sub.n * 1);
